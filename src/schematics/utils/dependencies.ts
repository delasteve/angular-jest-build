 ...
 */
import { JsonAstObject, JsonParseMode, parseJsonAst, JsonAstNode } from '@angular-devkit/core';
...
export function removePackageJsonDependency(tree: Tree, packageName: string): void {
  const packageJsonAst = _readPackageJson(tree);
  const recorder = tree.beginUpdate(pkgJsonPath);

  [NodeDependencyType.Default, NodeDependencyType.Dev, NodeDependencyType.Optional, NodeDependencyType.Peer].forEach(
    depType => {
      const depNode = findPropertyInAstObject(packageJsonAst, depType);
      if (!depNode || depNode.kind !== 'object') {
        return;
      }

      // const package = findPropertyInAstObject(depNode, packageName);

      // check if last entry in dependencies object
      // if last, remove last entry's comma
